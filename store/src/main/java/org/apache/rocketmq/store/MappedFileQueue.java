...
public class MappedFileQueue {
    ...
    /**
     * 这里说明一下mq file offset 结构，
     * 文件名称是fileFromOffset 左补充 一共20位的一个名称，大小默认为1G；举个例子，
     * 例如，
     *
     * 第一个MappedFile ：
     ...
     * @param cleanImmediately  是否立刻销毁 时间到了或者空间
     * @return
     */
    public int deleteExpiredFileByTime(final long expiredTime,
        ...
        if (null != mfs) {
            for (int i = 0; i < mfsLength; i++) {
                MappedFile mappedFile = (MappedFile) mfs[i];
                //存活最大时间戳 = 文件最后一次修改的时间戳 + 60 * 60 * 1000
        ...
        //从缓存中删除所有过期，也即上述步骤中，物理删除成功的MappedFile
     ...
     * 简单说一下该算法，先获取MappedFileQueued的首个MappedFile的fileFromOffset，也即是创建该MappedFile 的物理偏移量位置
     * ，而offset 为指定条消息的开始物理偏移量位置，该值是由当前所在文件的fileFromOffset + 前一条消息的最后一个字节所在的位置算出
     * 例如，第一个mappedFile的的fileFromOffset 为零 ，该mappedFile存储消息满以后，会创建第二个mappedFile，第二个mappedFile的fileFromOffset 为1073741824
     * ，第三个mappedFile的fileFromOffset 为2147483648；
