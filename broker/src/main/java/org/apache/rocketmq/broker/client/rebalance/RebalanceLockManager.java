...
public class RebalanceLockManager {
    ...
    /**
     * 根据group 与clientId 锁定 mq 集合
     * @param group 消费者组
     * @param mqs mq集合
     * @param clientId consumer客户端实例id  =  (consumer client)ipaddress + "@" + instanceName
     * @return clientId获取的mq锁集合，包括原来已经获取的和新的
     */
    public Set<MessageQueue> tryLockBatch(final String group, final Set<MessageQueue> mqs,
        ...
        for (MessageQueue mq : mqs) {
            //clientId 为consumer 的客户端id = address + instanceName;
            //LockEntry 为consumer端 clientId 的实体，该方法用来判断group下的mq是否被指定的clientId锁着
            //原来获取的
            if (this.isLocked(group, mq, clientId)) {
                lockedMqs.add(mq);
            } else {
