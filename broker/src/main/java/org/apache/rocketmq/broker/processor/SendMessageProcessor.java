...
public class SendMessageProcessor extends AbstractSendMessageProcessor implements NettyRequestProcessor {
    ...
    private RemotingCommand sendMessage(final ChannelHandlerContext ctx, //
        ...
        //?? 理论上不会出现System.currentTimeMillis() < 0
        ...
        response.setCode(-1);
        //先从本地缓存topicConfigTable 获取
        //如果获取不到则:
        //创建对应的topic2mq 信息，放到缓存中，然后持久化到硬盘。
        //然后广播向nameser注册并创建topic
        ...
        //tags 值存放在requestHeader.getProperties()  PROPERTY_TAGS 属性里，将producer客户端指定的String 类型的tags 转为long 类型的hashCode。
        ...
        //commit log-> put message; 里面实现了消息持久化功能以及主从消息同步等
        PutMessageResult putMessageResult = this.brokerController.getMessageStore().putMessage(msgInner);
        if (putMessageResult != null) {
